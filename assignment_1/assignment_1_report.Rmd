...
In the plot below one clearly observes two main differences between the Policy Paths from $K_0$ for $\sigma=1$ and $\sigma=5$. First, the steady state for the path with $\sigma=5$ is lower than the steady state with $\sigma=1$. Second, the Policy Path with $\sigma=1$ converges faster to its steady state than with $\sigma=5$. 

Note that the derivative $\frac{\partial u(c)}{\partial c}=c^{-\sigma}$ for $\sigma \not=1$ and $\frac{\partial u(c)}{\partial c}=c^{-1}$ for $\sigma=1$. That is, utility is increasing in consumption, but, for higher $\sigma$, the utility will increase slower than for lower $\sigma$.   
...
If we look at the emission path for $M_0$, we indeed observe this pattern. For $\beta=0.75$, one can clearly see that the emissions will be higher compared to the path for $\beta=0.85$. 
